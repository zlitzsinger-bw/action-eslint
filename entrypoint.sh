...
echo "filter mode: ${INPUT_FILTER_MODE} ${INPUT_FILTER_MODE}"
...
if [ "${INPUT_REPORTER}" == 'github-pr-review' ]; then
  ...
  $(npm bin)/eslint -f="json" ${INPUT_ESLINT_FLAGS:-'.'} \
    ...
    | reviewdog -efm="%f:%l:%c:%m" -name="eslint" -reporter=github-pr-review -level="${INPUT_LEVEL}" -filter-mode="${INPUT_FILTER_MODE}" 
else
